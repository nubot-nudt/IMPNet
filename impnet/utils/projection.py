...
class projection:
    ...
    def get_moving_points_from_range_view(self, output):

        mos_mask = output["motion_seg"].argmax(dim=2)
        mos_range_view = output["rv"].clone()

        # Set invalid points to -1.0 according to mask
        mos_range_view[mos_mask == 0] = -1.0
        return mos_range_view
    
    def get_gt_moving_points_from_range_view(self, mos_label,target_rv):

        # Set invalid points to -1.0 according to mask
        target_rv[mos_label == 0] = -1.0
        return target_rv
