...
class Visualization:
    ...
    def update(self, vis):
        ...
        gt_pcd, pred_pcd ,pred_mos_pcd, gt_mos_pcd= self.get_gt_and_predictions(
            self.path, self.sequence, self.current_frame, self.current_step
        )
        
        gt_pcd.paint_uniform_color([1, 0, 0])#蓝色
        gt_mos_pcd.paint_uniform_color([1, 0, 0])#红
        pred_pcd.paint_uniform_color([0, 0, 1]) #
        pred_mos_pcd.paint_uniform_color([0, 0, 1])#绿
        
        gt_mos= [gt_pcd,gt_mos_pcd]
        pred_mos = [pred_pcd,pred_mos_pcd]
        all = [gt_pcd,pred_pcd]
        mos = [gt_mos_pcd,pred_mos_pcd]
        geometry_list=all
        if self.car_mesh:
    ...
    def get_gt_and_predictions(self, path, sequence, current_frame, step):
        ...
        pred_mos_pcd = o3d.io.read_point_cloud(pred_move_path)
        ...
        gt_move_path = os.path.join(
            path, sequence, "gt_mos", str(current_frame + step).zfill(6) + ".ply"
        )
        gt_mos_pcd=o3d.io.read_point_cloud(gt_move_path)

        return gt_pcd, pred_pcd, pred_mos_pcd, gt_mos_pcd
