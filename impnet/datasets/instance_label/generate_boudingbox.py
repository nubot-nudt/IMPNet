...
def hbdPointsCluster(mapped_labels, scan):
    pointscluster = []
    inst_id = 1
    for sem_label in range(1, 9):
        ...
        if sem_label in mapped_labels:  # points with same semantic label
            ...
            if selected_points.shape[0] < min_cluster_size:
                continue
            #clusterer = hdbscan.HDBSCAN(min_cluster_size=10,cluster_selection_method='leaf')
            #cluster_labels = clusterer.fit_predict(selected_points)
            with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm:
                cluster_labels = np.array(pcd.cluster_dbscan(eps=0.5, min_points=5, print_progress=True))       

            for cluster_id in np.unique(cluster_labels):
                if cluster_id != -1: 
                    indices = np.where(cluster_labels == cluster_id)[0]
                    inst_cluster = selected_points[indices, 0:3]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_raw', type=str, default=None,
                        help='specify the point cloud data file or directory')
    parser.add_argument('--view',default='True', action='store_true', help='true for visualization')
    parser.add_argument('--save', action='store_true', help='true for saving the bounding box label')
    parser.add_argument('--lshape', action='store_true',default='True', help='using lshape to refine the bounding box, specify for car')
    ...
    save_dir = args.data
    save_folder = os.path.join(save_dir, 'boundingbox')
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    for frame_idx in tqdm(range(0,len(scan_paths))):
        scan = load_scan(scan_paths[frame_idx])
        sem_label,inst_label= load_labels(label_paths[frame_idx])
        ...
        background_color = np.zeros((len(background_selected),3),dtype=np.float64)
        background_color = background_color + 0.5
        back_ground_point.colors =o3d.utility.Vector3dVector(background_color)   

        # instance clustering
        
        pointcloud_cluster = hbdPointsCluster(mapped_labels, scan)
